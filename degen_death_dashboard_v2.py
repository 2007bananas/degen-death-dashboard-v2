...
<style>
    ...
    .panel { background: #1e2937; padding: 16px; border-radius: 8px; border: 1px solid #334155; }
...
with col3:
    if st.button("🔗 Connect Phantom"):
        ...
        st.success("Phantom Connected (Polygon)")

# Main Layout - Multi-Panel Style
col_left, col_center, col_right = st.columns([1.2, 2.5, 1.2])

with col_left:  # Watch List
    st.subheader("📋 Watch List")
    watch_items = ["BTC 5m Up", "ETH 5m Down", "BTC/USD", "EUR/USD", "Oil Brent"]
...
with col_center:  # Main Charting
    ...
    if st.button("🚀 EXECUTE BUY"):
        ...
        st.success(f"Executed BUY ${size} on {symbol}")
    if st.button("💀 EXECUTE SELL"):
        st.session_state.balance -= size * 0.65
        st.session_state.pnl_history.append(st.session_state.balance)
        st.success(f"Executed SELL ${size} on {symbol}")

with col_right:  # Positions & Orders
    st.subheader("📍 Positions")
    st.markdown('<div class="panel">EUR/AUD +100k (+$1,234)</div>', unsafe_allow_html=True)
...
# Tabs for additional depth
tab1, tab2, tab3 = st.tabs(["Global Risk", "Polymarket Edges", "Data Hub"])
...
with tab2:
    st.subheader("🔥 Polymarket 5-Min Edges")
    st.info("BTC 5-min Down — Edge +28% (High confidence)")

with tab3:
    st.subheader("📚 Data Hub")
    st.caption("Sources for AI training: yfinance, Alpha Vantage, Kaggle, FRED, Polygon.io, Finnhub, CRSP, Databento, CoinAPI")
